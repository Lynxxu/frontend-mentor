...
const Triangle = () => {return(
  ...
  >
    <path
      d="m2.095 19.882 9.248-16.5c.133-.237.384-.384.657-.384.272 0 .524.147.656.384l9.248 16.5c.064.115.096.241.096.367 0 .385-.309.749-.752.749h-18.496c-.44 0-.752-.36-.752-.749 0-.126.031-.252.095-.367zm1.935-.384h15.939l-7.97-14.22z"
      fillRule="nonzero"
    />
  </svg>);
};

const shapes = [Star, Triangle, Hexagon, Circle];
...
export default function BackgroundRain() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      const Component = shapes[Math.floor(Math.random() * shapes.length)];
      const color = colors[Math.floor(Math.random() * colors.length)];
      const duration = Math.random() * 10 + 5; // 5 to 15 seconds
      const rotation = Math.random() * 20 + 10; // 10 to 30 seconds
      const delay = Math.random() * 2; // 0 to 2 seconds before start
      const key = `${new Date().getTime()}-${Math.random()}`;

      const geom = { Component, color, duration, delay, rotation, key };
      setGeometries(current => [...current, geom]);

      // Optional: Cleanup if too many elements
      if (geometries.length > 50) {
        setGeometries(current => current.slice(10));
      }
    }, 1500);
  ...
  return (
    <div className="fixed inset-0 overflow-hidden pointer-events-none">
      {geometries.map(({ key, Component, color, duration, delay, rotation }) => (
        <div key={key} style={{
          position: 'absolute',
